/** AUTO-GENERATED from src/preload/index.ts — do not edit. Run: pnpm gen:window-api */
...
const api = {
  ...
  select: (options: OpenDialogOptions) => ipcRenderer.invoke(IpcChannel.App_Select, options),
  ...
  tree: {
    ...
    onMutation: (callback: (payload: TreeMutationPushPayload) => void) => {
      const listener = (_event: any, payload: TreeMutationPushPayload) => {
  ...
  mcp: {
    ...
    onServerLog: (callback: (log: McpServerLogEntry & { serverId?: string }) => void) => {
      const listener = (_event: any, log: McpServerLogEntry & { serverId?: string }) => {
        callback(log)
      }
  ...
  protocol: {
    onReceiveData: (callback: (data: { url: string; params: any }) => void) => {
      const listener = (_event: any, data: { url: string; params: any }) => {
        callback(data)
      }
  ...
  webview: {
    ...
    onFindShortcut: (callback: (payload: WebviewKeyEvent) => void) => {
      const listener = (_event: any, payload: WebviewKeyEvent) => {
        callback(payload)
      }
  ...
  shortcut: {
    onRegistrationConflict: (callback: (payload: ShortcutRegistrationConflictPayload) => void): (() => void) => {
      ...
      const listener = (_: any, payload: ShortcutRegistrationConflictPayload) => callback(payload)
  ...
  ai: {
    ...
    onStreamChunk: (callback: (data: StreamChunkPayload) => void) => {
      const listener = (_: any, data: StreamChunkPayload) => callback(data)
    ...
    onStreamDone: (callback: (data: StreamDonePayload) => void) => {
      const listener = (_: any, data: StreamDonePayload) => callback(data)
    ...
    onStreamError: (callback: (data: StreamErrorPayload) => void) => {
      const listener = (_: any, data: StreamErrorPayload) => callback(data)
  ...
  lanTransfer: {
    ...
    onServicesUpdated: (callback: (state: LanTransferState) => void): (() => void) => {
      ...
      const listener = (_: any, state: LanTransferState) => callback(state)
      ipcRenderer.on(channel, listener)
      return () => {
        ipcRenderer.removeListener(channel, listener)
      }
    ...
    onClientEvent: (callback: (event: LanClientEvent) => void): (() => void) => {
      ...
      const listener = (_: any, event: LanClientEvent) => callback(event)
      ipcRenderer.on(channel, listener)
      return () => {
        ipcRenderer.removeListener(channel, listener)
      }
