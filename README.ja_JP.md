...
- [x] **Menu Bar**：メニューバーにあすみに関連したアイコンを表示し、クリックで機能を提供。
- [x] **マルチモデル対応**：複数のモデル（API / ローカル）に対応し、さまざまな対話・インタラクションに対応。
...
- [x] **クイック起動**：グローバルホットキーでミニウィンドウを呼び出し、あすみと素早く対話。
- [ ] **情報取得**：ユーザーが指定した情報源（ウェブサイトやソーシャルメディアなど）から定期的に情報を取得し、要約して提供。
- [ ] **ファイル処理**：ファイルをドラッグ＆ドロップで書記あすみに渡し、スクリプトやスキルで自動仕分け。
- [x] **Asumi Skill**：ユーザーがあすみの口調やスタイルを指定して出力をフォーマット。
- [ ] **Third Party Skill**：ユーザーがサードパーティのスキルを統合し、あすみの機能を拡張可能。
